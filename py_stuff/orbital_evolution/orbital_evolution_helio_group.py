...
CC_size=30 # marker size for crosses to denote the point is in the Cold Classical region
survivor_lw=1 # linewidth to highlight survivors
binary_load=0 # do we load binary data from files?
# s1=25 # marker size for tight binaries
# s2=90 # marker size for wide binaries
s1=5 # marker size for tight binaries
s2=20 # marker size for wide binaries
s3=s1 # marker size for singles
no_survive=0 # If there are no survivors, equals 1
abm=1.0
asm=0.5 # alpha for single markers
al=0.1 # alpha for connecting lines
#-------------------------------------------------------------------------------
...
for i in range(len(df_tau)):
    ...
    ax4.set_xlabel('a (AU)')
    fig.suptitle('migration parameters: {}'.format(vals))
    ...
    ax3.plot([a_min,a_min],[0.0,e_min],'k',zorder=2)
    ax3.plot([a_max,a_max],[0.0,e_max],'k',zorder=2)
    ax4.plot([a_min,a_min],[I_min,I_max],'k',zorder=2)
    ax4.plot([a_max,a_max],[I_min,I_max],'k',zorder=2)
    ax4.plot([a_min,a_max],[I_max,I_max],'k',zorder=2)
    ...
    ax3.plot(a_lim,e_lim,'k',zorder=2)
    ...
    for run in dirs:
        ...
        for k in range(len(files)):
            ...
            ax1.scatter(df_orb1['a_hel(m)']/pf.AU,df_orb1['e_hel'],label="t={}Myr".format(t),s=s1,color=col,zorder=1,alpha=abm)
            ax2.scatter(df_orb1['a_hel(m)']/pf.AU,df_orb1['I_hel(rad)']/(numpy.pi/180.0),label="N_binaries={}".format(len(a_hel)),s=s1,color=col,zorder=1,alpha=abm)
            ax3.scatter(df_orb1['a_hel(m)']/pf.AU,df_orb1['e_hel'],label="t={}Myr".format(t),s=s1,color=col,zorder=1,alpha=abm)
            ax4.scatter(df_orb1['a_hel(m)']/pf.AU,df_orb1['I_hel(rad)']/(numpy.pi/180.0),label="N_binaries={}".format(len(a_hel)),s=s1,color=col,zorder=1,alpha=abm)
            # wide binaries
            df_orb2=df_orb[df_orb['a/r_hill']>=wide_cut]
            print "number of wide binaries = {}".format(len(df_orb2))
            ax1.scatter(df_orb2['a_hel(m)']/pf.AU,df_orb2['e_hel'],s=s2,color=col,label=None,zorder=1,alpha=abm)
            ax2.scatter(df_orb2['a_hel(m)']/pf.AU,df_orb2['I_hel(rad)']/(numpy.pi/180.0),s=s2,color=col,label=None,zorder=1,alpha=abm)
            ax3.scatter(df_orb2['a_hel(m)']/pf.AU,df_orb2['e_hel'],s=s2,color=col,label=None,zorder=1,alpha=abm)
            ax4.scatter(df_orb2['a_hel(m)']/pf.AU,df_orb2['I_hel(rad)']/(numpy.pi/180.0),s=s2,color=col,label=None,zorder=1,alpha=abm)
        ...
        ax1.scatter(df_orb_survivor1['a_hel(m)']/pf.AU,df_orb_survivor1['e_hel'],edgecolor='k',facecolors='none',lw=survivor_lw,label=None,s=s1,zorder=2)
        ax2.scatter(df_orb_survivor1['a_hel(m)']/pf.AU,df_orb_survivor1['I_hel(rad)']/(numpy.pi/180.0),edgecolor='k',facecolors='none',lw=survivor_lw,label=None,s=s1,zorder=2)
        ax3.scatter(df_orb_survivor1['a_hel(m)']/pf.AU,df_orb_survivor1['e_hel'],edgecolor='k',facecolors='none',lw=survivor_lw,label=None,s=s1,zorder=2)
        ax4.scatter(df_orb_survivor1['a_hel(m)']/pf.AU,df_orb_survivor1['I_hel(rad)']/(numpy.pi/180.0),edgecolor='k',facecolors='none',lw=survivor_lw,label=None,s=s1,zorder=2)
        df_orb_survivor2=df_orb_survivor[df_orb_survivor['a/r_hill']>=wide_cut]
        print len(df_orb_survivor2)
        ax1.scatter(df_orb_survivor2['a_hel(m)']/pf.AU,df_orb_survivor2['e_hel'],edgecolor='k',facecolors='none',lw=survivor_lw,label=None,s=s2,zorder=2)
        ax2.scatter(df_orb_survivor2['a_hel(m)']/pf.AU,df_orb_survivor2['I_hel(rad)']/(numpy.pi/180.0),edgecolor='k',facecolors='none',lw=survivor_lw,label=None,s=s2,zorder=2)
        ax3.scatter(df_orb_survivor2['a_hel(m)']/pf.AU,df_orb_survivor2['e_hel'],edgecolor='k',facecolors='none',lw=survivor_lw,label=None,s=s2,zorder=2)
        ax4.scatter(df_orb_survivor2['a_hel(m)']/pf.AU,df_orb_survivor2['I_hel(rad)']/(numpy.pi/180.0),edgecolor='k',facecolors='none',lw=survivor_lw,label=None,s=s2,zorder=2)

        # plot all single objects
        ax1.scatter(numpy.array(a_hel_single)/pf.AU,e_hel_single,c='k',alpha=asm,edgecolors='none',marker='^',zorder=1,s=s3)
        ax2.scatter(numpy.array(a_hel_single)/pf.AU,numpy.array(I_hel_single)/(numpy.pi/180.0),c='k',alpha=asm,edgecolors='none',label="N_singles={}".format(len(a_hel_single)),marker='^',zorder=1,s=s3)
        ax3.scatter(numpy.array(a_hel_single)/pf.AU,e_hel_single,c='k',alpha=asm,edgecolors='none',marker='^',zorder=1,s=s3)
        ax4.scatter(numpy.array(a_hel_single)/pf.AU,numpy.array(I_hel_single)/(numpy.pi/180.0),c='k',alpha=asm,edgecolors='none',label="N_singles={}".format(len(a_hel_single)),marker='^',zorder=1,s=s3)

        # plot a line to show evolution from initial to final positions
        for j in range(len(a_hel)):
            ax1.plot(numpy.array([a_hel_survivor[j],a_hel[j]])/pf.AU,[e_hel_survivor[j],e_hel[j]],color='k',alpha=al,zorder=0)
            ax2.plot(numpy.array([a_hel_survivor[j],a_hel[j]])/pf.AU,numpy.array([I_hel_survivor[j],I_hel[j]])/(numpy.pi/180.0),color='k',alpha=al,zorder=0)
            ax3.plot(numpy.array([a_hel_survivor[j],a_hel[j]])/pf.AU,[e_hel_survivor[j],e_hel[j]],color='k',alpha=al,zorder=0)
            ax4.plot(numpy.array([a_hel_survivor[j],a_hel[j]])/pf.AU,numpy.array([I_hel_survivor[j],I_hel[j]])/(numpy.pi/180.0),color='k',alpha=al,zorder=0)
