...
n_order=2000 # factor for scaling histgram on plot
n_bins=10
#-------------------------------------------------------------------------------
...
for i in range(len(df_tau)):
    ...
    for run in dirs:
            ...
            if binary_load==1:
                ...
                if k==0:
                    ...
                    while _i < len(pos)-1:
                        ...
                        a_bin_0.append(dat[0,1])
                        e_bin_0.append(dat[0,2])
                        I_bin_0.append(dat[0,3])
                ...
                if k!=0:
                    ...
                    for _i in primary_list:
                        ...
                        if a[_i]>0.0 and a[_i]<a_hel_lim and e[_i]<1.0: # check helio orbits
                            ...
                            a_bin_1.append(dat[-1,1])
                            e_bin_1.append(dat[-1,2])
                            I_bin_1.append(dat[-1,3])
                ...
                if k==0:
                    ...
                    a_bin_0.extend(a_bin)
                    e_bin_0.extend(e_bin)
                    I_bin_0.extend(I_bin)
                    ...
                    a_bin_1.extend(a_bin)
                    e_bin_1.extend(e_bin)
                    I_bin_1.extend(I_bin)
        ...
        ax1.scatter(df_orb_survivor1['a_bin(m)'],df_orb_survivor1['e_bin'],edgecolor='k',facecolors='none',lw=survivor_lw,label=None,s=s1,alpha=abm)
        ax2.scatter(df_orb_survivor1['a_bin(m)'],df_orb_survivor1['I_bin(rad)']/(numpy.pi/180.0),edgecolor='k',facecolors='none',lw=survivor_lw,label=None,s=s1,alpha=abm)
        df_orb_survivor2=df_orb_survivor[df_orb_survivor['a/r_hill']>=wide_cut]
        print "len df_orb_survivor2={}".format(len(df_orb_survivor2))
        ax1.scatter(df_orb_survivor2['a_bin(m)'],df_orb_survivor2['e_bin'],edgecolor='k',facecolors='none',lw=survivor_lw,label=None,s=s2,alpha=abm)
        ax2.scatter(df_orb_survivor2['a_bin(m)'],df_orb_survivor2['I_bin(rad)']/(numpy.pi/180.0),edgecolor='k',facecolors='none',lw=survivor_lw,label=None,s=s2,alpha=abm)

        # plot a line to show evolution from initial to final positions
        if k==1:
            for j in range(len(a_bin)):
                ax1.plot([a_bin_survivor[j],a_bin[j]],[e_bin_survivor[j],e_bin[j]],color='k',alpha=al,zorder=2)
                ax2.plot([a_bin_survivor[j],a_bin[j]],[I_bin_survivor[j],I_bin[j]],color='k',alpha=al,zorder=2)

    I_bin_0=numpy.array(I_bin_0)/(numpy.pi/180.0) # convert to degrees
    I_bin_1=numpy.array(I_bin_1)/(numpy.pi/180.0) # convert to degrees

    print len(a_bin_0)
    hist_scale_e = numpy.amax(a_bin_0)/n_order# 0.25e6
    hist_scale_I = numpy.amax(a_bin_0)/n_order

    # Add eccentricity histogram for initial conditions
    center, hist, width=pf.hist_dist(e_bin_0,n_bins)
    hist=hist*hist_scale_e
    ax1.barh(y=center, height=width, width=hist, align='center', color=colours[0], edgecolor='k',zorder=0,alpha=0.5)#,label='t={}Myr'.format(t))
    ...
    #
    # Add inclination histogram for initial conditions
    center, hist, width=pf.hist_dist(I_bin_0,n_bins)
    hist=hist*hist_scale_I
    ax2.barh(y=center, height=width, width=hist, align='center', color=colours[0], edgecolor='k',zorder=0,alpha=0.5)#,label='t={}Myr'.format(t))

    # Add eccentricity histogram for final states
    center, hist, width=pf.hist_dist(e_bin_1,n_bins)
    hist=hist*hist_scale_e
    # plot bars on the left
    ax1.barh(y=center, height=width, width=hist, align='center', color=colours[1], edgecolor='k',zorder=0,alpha=0.5)#,label='t={}Myr'.format(t))

    # Add inclination histogram for final states
    center, hist, width=pf.hist_dist(I_bin_1,n_bins)
    hist=hist*hist_scale_I
    # plot bars on the left
    ax2.barh(y=center, height=width, width=hist, align='center', color=colours[1], edgecolor='k',zorder=0,alpha=0.5)#,label='t={}Myr'.format(t))
